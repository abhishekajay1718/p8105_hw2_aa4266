...
```
There are `r  count(distinct(nyc_transit_data[which(nyc_transit_data$ada == TRUE),], line, station_name))`
...
```{r p1_q3}
stn_wo_vending_w_entrance_porportion =
  nyc_transit_data %>%
  filter(vending == "NO", entry == TRUE) %>%
  count()/nrow(nyc_transit_data[which(nyc_transit_data$vending == "NO"),]) 
```

The proportion of station entrances/exits without vending that allow entrance are `r stn_wo_vending_w_entrance_porportion`
