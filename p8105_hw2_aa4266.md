...
``` r
mr_trash_wheel_data =
  read_excel("./datasets/HealthyHarborWaterWheelTotals2018-7-28.xlsx", range = "A2:O338") %>%
  janitor::clean_names() %>%
  filter(is.na(x_1)) %>%
  select(-x_1) %>%
...
mr_trash_wheel_precip_data_2016$month[13] = "Grand Total"
...
mr_trash_wheel_precip_data_2017$month[13] = "Grand Total"
```
...
Write some stuff about problem 2
--------------------------------
...
#   select(excellent) %>%
#   median( na.rm = TRUE)
#   selecting a single column, na.rm = T
```
